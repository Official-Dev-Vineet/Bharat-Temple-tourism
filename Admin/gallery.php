...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["gallery"])) {
    ...
    // Define target directory to save uploaded image
    // Create table 
    $targetDir = "uploads/";
    ...
    if (in_array($imageFileType, $allowedExtensions)) {
        ...
        if (move_uploaded_file($_FILES["gallery"]["tmp_name"], $targetFilePath)) {
            ...
            $stmt->bindParam(':name', $_SESSION['AdminName'], PDO::PARAM_STR);
            $stmt->bindParam(':file_name', $fileName, PDO::PARAM_STR);
            $stmt->bindParam(':file_path', $targetFilePath, PDO::PARAM_STR);
            $stmt->bindParam(':email', $_SESSION['AdminEmail'], PDO::PARAM_STR);
            ...
            if ($stmt->execute()) {
                ...
                echo "Error saving file details in database";
                exit;
            }
        } else {
        ...
        echo "Invalid file format: " . $fileName;
        exit;
    }
        ...
        <div class="container">
            ...
            <form id="uploadForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="review" name="review" rows="3" maxlength="200" required></textarea>
                    <small id="reviewHelp" class="form-text text-muted">You have <span id="charCount">200</span> characters remaining.</small>
                </div>
