...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["title"], $_POST["location"], $_POST["description"]) && isset($_FILES["image"])) {
    ...
    if ($_FILES["image"]["size"] > 500000) {
        ...
        // delete inserted data from database
        $deleteSql = "DELETE FROM populardestination WHERE id = :entry_id";
        $stmtDelete = $conn->prepare($deleteSql);
        $stmtDelete->bindParam(':entry_id', $entryId, PDO::PARAM_INT);
        $stmtDelete->execute();
        $uploadOk = 0;
        exit;
    }
        ...
        <div class="container mt-5">
            <div class="row">
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo '<h5 class="card-title text-dark">' . $title . '</h5>';
                    echo '<p class="card-text text-dark"><strong>Location:</strong> ' . $location . '</p>';
                    echo '<p class="card-text text-dark"><strong>Description:</strong> ' . $description . '</p>';
                    echo '<button onclick=deleteCard(' . $id . ') class="btn btn-danger">Delete</button>';
                    // add updateBtn=
                    echo '<button onclick=window.location.href="updatePopular.php?id=' . $id . '" class="btn btn-primary ml-2">Update</button>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
                }
