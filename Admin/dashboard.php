    ...
    try {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                ...
                $stmt = $conn->prepare("INSERT INTO tourpackages (title, description, image, pax, location, day, price, reviewCount, tourId, status, include, exclude, schedule) 
                                    VALUES (:title, :description, :image, :pax, :location, :day, :price, :reviewCount, :tourId,:status, :include, :exclude, :schedule)");

                // Bind parameters
                $tourId=uniqid("BHARAT");
                ...
                // Explode include and exclude into arrays
                $includeArr = explode("\n", $_POST['include']);
                $excludeArr = explode("\n", $_POST['exclude']);
                $include = implode("\n", $includeArr);
                $exclude = implode("\n", $excludeArr);
                $stmt->bindParam(':include', $include);
...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="container mt-5">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data" id="tourForm">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="include" name="include" rows="5" required></textarea>
                </div>
                <div class="form-group">
                    <label for="exclude">Exclude:</label>
                    <textarea class="form-control" id="exclude" name="exclude" rows="5" required></textarea>
                </div>

                <div class="form-group">
                    <label for="schedule">Tour Schedule:</label>
                    <textarea class="form-control" id="schedule" name="schedule" rows="5" required></textarea>
                </div>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr id="tourpackage<?php echo $tourpackage["id"]; ?>" class="text-light">
                                    ...
                                    <td><img src="<?php echo $tourpackage["image"]; ?>" width="100" alt="tour package image"></td>
