...
<body>
    ...
    <div class="container-fluid">
        <div class="container mt-5">
            <h2 class="text-center text-light">Booking Details</h2>
            <div class="table-responsive bg-secondary">
                <table class="table table-striped text-center align-middle">
                    <thead>
                        <tr class="text-nowrap text-light bg-dark">
                            <th>Sr No.</th>
                    ...
                    <tbody class="bg-dark">
                        ...
                        $sql = "SELECT * FROM bookingtour WHERE emailId = '$_SESSION[email]'";
                        $stmt = $conn->prepare($sql);
                        $stmt->execute();
                        $srno = 0;
                        while ($row = $stmt->fetch()) {
                            if (count($row) > 0 && $row['status'] == 'booked') {
                                ...
                                echo "<td><a href='cancel_tour.php?id=" . $row['pnr'] . "' class='btn btn-danger my-2'>Cancel</a>
                            <a href='updateBooking.php?id=" . $row['pnr'] . "&tourId=" . $row['tourId'] . "' class='btn btn-primary'>Update</a>
                        </td>";
                                echo "</tr>";
                            }
                        }
